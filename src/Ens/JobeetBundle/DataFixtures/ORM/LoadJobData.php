...
class LoadJobData extends AbstractFixture implements OrderedFixtureInterface
{
  public function load(ObjectManager $em) {
    ...
    $job_expired_sensio = new Job();
    $job_expired_sensio->setCategory($em->merge($this->getReference('category-design')));
    $job_expired_sensio->setType('part-time');
    $job_expired_sensio->setCompany('Extreme Sensio');
    $job_expired_sensio->setLogo('extreme-sensio.gif');
    $job_expired_sensio->setUrl('http://www.extreme-sensio.com/');
    $job_expired_sensio->setPosition('Web Designer expired');
    $job_expired_sensio->setLocation('Paris, France');
    $job_expired_sensio->setDescription('Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in.');
    $job_expired_sensio->setHowToApply('Send your resume to fabien.potencier [at] sensio.com');
    $job_expired_sensio->setIsPublic(true);
    $job_expired_sensio->setIsActivated(true);
    $job_expired_sensio->setToken('job_extreme_sensio');
    $job_expired_sensio->setEmail('job@example.com');
    $job_expired_sensio->setCreatedAt(new \DateTime('2012-10-10'));
